...
| `/cpp-distill <source> [--global] [--force]` · `/cpp-distill check <path>` · `/cpp-distill distill <source>` | cmd | Atomic-ingest + 22-section distillation (Tandas T1/T2/T3 × Partes I/II/III) + IRE-7 materialization. `distill` = zero-token deterministic synthesizer via the canonical KobiiCraft engine (default path); `check` validates an existing output dir; bare/`ingest` = in-session driver against the Mother Prompt. | 1 MB cap; secret redaction + code-fence-aware placeholder reject; voice gate enforcing (exit 5); `distill` needs `$KOBII_DISTILLER_ENGINE_ROOT` and fails LOUD if unset (no silent LLM fallback). | `<output_root>/Tier_N/Seccion_N.md` × 22 + ingest sidecar + READY.md. | `commands/distill.md` |
...
| `/cpp-resume-sovereign <query\|composer_id\|--list>` | cmd | Fuzzy-search the Sovereign History Vault (46k+ records) and inject a recovered session as `<retrieved-history readonly="true">` context + project-filtered Universal Laws. | FTS5 BM25 ranked; dedup'd picker (excludes composers open in another Cursor pane via 30s-cached scan); read-only injection (no native /resume bridge). | Markdown block w/ retrieved bubbles + matching laws. | `commands/resume-sovereign.md` |
