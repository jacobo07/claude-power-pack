...
| `/cpp-distill <source> [--global] [--force]` · `/cpp-distill check <path>` | cmd | Atomic-ingest + 22-section distillation (Tandas T1/T2/T3 × Partes I/II/III) + IRE-7 materialization, driven in-session against the Mother Prompt. `check` subcommand validates an existing output dir against the schema. | 1 MB cap; secret redaction + code-fence-aware placeholder reject; voice gate enforcing (exit 5); in-session driver (no external API). | `<output_root>/Tier_N/Seccion_N.md` × 22 + ingest sidecar + READY.md. | `commands/distill.md` |
...
| `distiller` | mod | KobiiDistillerOS v1.0 Sovereign Sealing — 3-Tier / **22-section** destilation pipeline with Tandas & Partes structural layer (T1/T2/T3 × I/II/III), kernel vMAX-NULL-ERROR (placeholder + ROI + Hawkins + voice gates), Markdown emitter, UKDL bridge, vaccine synth. | All 22 section titles ratified; validator enforces structural markers + global voice gate (blacklist+anchor); DRY-RUN default until ANTHROPIC_API_KEY set + Owner flips `distiller_dry_run=false`. | `Tier_N/Seccion_N.md` × 22 + ledger row + vaccines. Backend at `kobicraft_content_intelligence/distiller/`. | `modules/distiller/` |
