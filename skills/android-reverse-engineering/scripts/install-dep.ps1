...
function Install-Java {
    ...
    if ($javaBin) {
        # PP fix (see ../NOTICE.md): PS 5.1 wraps native stderr in an
        # ErrorRecord, which throws under $ErrorActionPreference='Stop'.
        $verOutput = (cmd /c "java -version 2>&1") | Select-Object -First 1
    ...
    $javaBin = Get-Command java -ErrorAction SilentlyContinue
    if ($javaBin) {
        Write-Ok "Java installed: $((cmd /c "java -version 2>&1") | Select-Object -First 1)"
    } else {
