...
if ($javaBin) {
    # PP fix (see ../NOTICE.md): on Windows PowerShell 5.1, redirecting a
    # native exe's stderr with 2>&1 wraps each line in an ErrorRecord
    # (NativeCommandError). Under $ErrorActionPreference='Stop' that THROWS,
    # and `java -version` writes to stderr -- so this aborted the whole check
    # for anyone who actually has Java installed. Letting cmd merge the
    # streams keeps PowerShell from ever seeing a native stderr write.
