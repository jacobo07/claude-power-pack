...
const {
  buildPayload: buildTerminalPayload,
  registryKey,
} = require("./terminal_registry");

const TERMINALS_DIR = path.join(os.homedir(), ".claude", "state", "terminals");
// Terminal open/close arrives in bursts -- a folderOpen revival launches one task per
// pane at once -- so coalesce into a single write instead of one write per event.
const TERMINAL_DEBOUNCE_MS = 300;
...
// wrote one (measured 2026-08-11: CursorProjects 6 open / 4 counted, Jacobo 2 / 3).
// See terminal_registry.js for the per-window file rationale.
// Fail-open ABSOLUTE: any failure writes nothing and the pane map keeps inferring
// openness from beacons exactly as before -- degraded, never wrong-by-addition.
async function captureTerminals() {
  try {
    ...
    for (const t of vscode.window.terminals || []) {
      ...
      rows.push({ name: t.name, processId: typeof pid === "number" ? pid : null });
    }
    const payload = buildTerminalPayload(cwd, rows, process.pid, new Date().toISOString());
    fs.mkdirSync(TERMINALS_DIR, { recursive: true });
    registryPath = path.join(TERMINALS_DIR, registryKey(cwd) + ".json");
    const tmp = registryPath + ".tmp";
    fs.writeFileSync(tmp, JSON.stringify(payload, null, 2), "utf8");
    fs.renameSync(tmp, registryPath);
  } catch (_e) {
    // never disrupt the editor; pane_map falls back to beacon-only liveness
  }
}

function scheduleTerminalCapture() {
...
function activate(context) {
  ...
  // Live terminal registry. tabGroups above enumerates EDITOR tabs only, so a
  // terminal in the bottom panel is invisible to it -- measured: tab_order.json held
  // 0 tabs while 21 panes were open. onDidOpenTerminal / onDidCloseTerminal are the
  // events that actually fire for those, so the count updates in real time.
  captureTerminals();
  try {
    context.subscriptions.push(
      vscode.window.onDidOpenTerminal(() => scheduleTerminalCapture()),
      vscode.window.onDidCloseTerminal(() => scheduleTerminalCapture())
    );
  } catch (_e) {
...
function deactivate() {
  // Drop this window's registry on a clean shutdown so a closed window stops having
  // its terminals counted. A crash skips this, which is exactly why the consumer
  ...
  try {
    ...
    // best effort; a stale file is already handled by the hostPid liveness check
  }
}
