...
STOP: Refuse GENUINELY_NEW_DATASET status until all 13 questions (problem owner gap, new outcome, real consumer, extension insufficiency, new primitive, new decisions, new evidence, failure class prevented, interfaces with existing owners, value measurement, complexity introduced, retirement condition, why-not-rhetorical-layer) are answered with cited file:line evidence from a DISCOVERED sweep of the repo -- never from the proposal's own curated list of what it assumes doesn't exist. Missing evidence on any question -> classify as EXTEND_EXISTING_OWNER, NEW_MODULE, NEW_VIEW, NEW_POLICY_PACK, NEW_SCANNER_OR_GATE, or REJECT instead. EXCEPCIÓN: Owner phrase "novelty proof waived -- HR-NOVELTY-001 OK" for ONE turn.
