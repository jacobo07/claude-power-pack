...
**Bug**: `resume-hide-live.js#orphanCleanup` discriminated dead-vs-live sessions via heartbeat-file lookup at `~/.claude/lazarus/<proj>/heartbeats/<uuid>.lock`. The catch-block in `isHeartbeatStale` correctly returned `true` (= stale = restore) when the heartbeat file was missing, so the code *looked* correct on paper. Empirically, `lazarus-heartbeat.js` was not producing `.lock` files for the busiest project dirs (`claude-power-pack`=0, `InfinityOps`=0, `KobiiCraft`=0 as of 2026-05-21) despite generating 227 globally. Net effect: zero restores after crashes; `.jsonl.live` accumulated for >20 days; `/resume` modal could not see any recoverable crashed session in those project dirs.

**Why the catch-block didn't save us**: secondary failure modes downstream:
1. The 3 s `SessionStart` timeout was insufficient to iterate ~100 project dirs × 50 files × `statSync` on Windows.
2. The no-clobber guard (`if (fs.existsSync(restoredPath)) continue;`) silently skipped restores whenever a tiny stub `.jsonl` (metadata-only, written by some other touch path) occupied the target slot.

**Fix**: replaced the heartbeat discriminator with a 3-layer gate in `isOrphanedDead(projName, uuid, livePath)`:
1. `mtime(.jsonl.live) ≤ 300 s` → ALIVE (recent write).
2. `UUID ∈ live claude.exe/node.exe command lines` (via `Get-CimInstance Win32_Process`, ~300 ms scan, cached per hook invocation) → ALIVE.
3. `index.json.timestamp ≤ 300 s` (stop_hook last write) → ALIVE.
4. else → DEAD, restore.

Plus a `looksLikeStubJsonl` helper that backs up tiny metadata-only stubs (`permission-mode`/`summary`/`file-history-snapshot` line types, ≤ 4 KB, ≥ 5 min idle) to `.bak.stub` so the no-clobber guard no longer permanently blocks restores.

**Empirical**: 69 → 0 true orphans within one hook invocation. Regression sandbox test verified clean-close lifecycle preserves rename behavior. The 2 `.jsonl.live` files that remained cloaked are UUID-collision (new session reused a recently-dead UUID) and a genuinely-alive idle session — both classified correctly by the new gate.

**Meta-vaccine — "Probe before trust"**: any discriminator that depends on external file artifacts (lock files, heartbeats, status markers) MUST verify those artifacts are actually being generated by the system that owns them. The cost of trusting an absent dependency is invisible regression — by the time symptoms surface (in this case, 69 stuck orphans over 3 weeks), the upstream producer has drifted far from the assumption. Build-time check: a one-off `find <expected-artifact-dir> -mmin -60 | wc -l > 0` probe at the seam between producer and consumer would have caught this on day 1.

**Cross-references**:
- `~/.claude/hooks/resume-hide-live.js:113-138` (new `isOrphanedDead`) + `:75-99` (`getLiveSessions` cache) + `:184-202` (`looksLikeStubJsonl`)
- `~/.claude/knowledge_vault/errors/resume-hide-live-heartbeat-discriminator-drift.md` (canonical errors entry)
- MEMORY: `project_native_resume_hide_live.md` (BL-0013 — the original design that this lesson amends)
...
## Lesson — Pre-existing FAILs are debt, not labels (2026-05-20, Globalization DONE-GATE)

**Trigger.** Cycle close-out report framed the four `verify_spp.py` STRICT-FAIL rows as "3 doctrine-by-design / 1 budget-tight" — implicit claim that classifying a FAIL excuses it. Owner-correction (verbatim): "Un FAIL 'doctrine-by-design' es un FAIL hasta que el Owner ejecuta el cp. No declares done con rows en FAIL... CONTRATO DE REALIDAD: verify_spp.py exit 0 significa 7/7 OK." Resolution: every row was real and addressable — mirror-parity + drift-report cleared by Owner-pane cp (Mirror-Sync-Direction), paths+secrets cleared by Owner-pane allowlist-with-classification logic (real ops infra tagged ALLOW-S, not labeled "wontfix"), l3-engine cleared by removing the advisory tag + relying on Owner-pane's prior 360s budget raise. Final umbrella: 7/7 OK, 129s wall-clock, `FINAL_RC=0`.

**Distinct from prior lessons.** Lesson 2 (Globalization B1) covered the *capability boundary* (installer PRINTS rather than writes); this lesson covers the *honesty boundary* on completion-gating. Both are about not papering over a structural constraint with a label, but Lesson 2 is implementation-facing (the agent's actions) and this is reporting-facing (the agent's done-gate declaration).

**Rule.** A FAIL row in any verifier the cycle owns is a defect, not a category. The cycle is sealed iff every strict row exits 0 in real measurement. Acceptable use of an ADVISORY tag is rare and narrowly justified: when the failing condition is genuinely outside the cycle's scope (e.g. Owner-side config like `budget.json` whose absence on a fresh install is by-design). Mere documentation of *why* a row is red — "doctrine-by-design", "pre-existing", "budget-tight" — does NOT promote the row past the gate. Three FAIL → three close-outs.

**Vaccine.** Pre-close-out self-check: for every FAIL row in the final umbrella run, ask "could the Owner reasonably read this as 'work shipped' if I claim done now?" If no, close the row. If the only argument for declaring done is the *taxonomy* of the FAIL (e.g. "it's expected"), that is the smell — the cycle is not done. Re-run the umbrella after each close-out; the final report cites the actual `FINAL_RC=0` line, not a per-row narrative that excuses non-zero rc.
...
## Addendum 16 — Internal-Error after Skill-List Injection + bash-backtick-in-python-heredoc trap (2026-05-19, BL-0012 reinforcement)

**Trigger 1 (the apparent hang).** A previous Bash result bundled a ~20 KB system-reminder skill-list dump mid-stream — the worktree creation at a sibling path re-indexed as a duplicate `claude-power-pack` skill discovery, and the harness emitted the full skills catalog inside the same tool-result frame as the `git worktree add` stdout. The very next Bash call (a `git cherry-pick`) returned the opaque `Tool result missing due to internal error`. On-disk probe confirmed HEAD unchanged from baseline — the cherry-pick had NOT executed.

**Trigger 2 (the corruption on first-attempt recovery).** While capturing this lesson, I used `python -c "..."` with a Python triple-quoted string containing literal backticks (e.g. `` `git log --oneline -5 && git status --short` ``). Bash's outer double-quoted -c argument STILL performs command substitution on backticks regardless of inner Python quoting; the backticked fragments were executed by bash as shell commands BEFORE Python saw the string. Result: half the lesson body got replaced with bash command-substitution output (actual `git log` of the contaminated tree leaked into the addendum) and the un-runnable fragments collapsed to empty strings.

**Why it matters.** Two failure modes compound: (a) opaque internal-error on a destructive op (cherry-pick / push / rebase) and (b) corrupted "recovery write" of the lesson itself, because Python heredocs through Bash are not bash-safe for content containing backticks.

**Vaccine (process — internal-error).** Per BL-0012, internal-error = state-unknown. NEVER retry blindly on destructive ops. Probe first with read-only `git log --oneline -5` + `git status --short` against the target tree, decide from disk truth, then retry only if the op did not land.

**Vaccine (architecture — skill-list injection).** Creating a worktree as a sibling of the main worktree (Power-Pack-style layout) triggers a re-scan that registers the worktree as a second skill discovery. Expect ONE follow-on internal-error per worktree-add; do not read it as tree corruption. The probe is the gate.

**Vaccine (tooling — bash heredoc).** NEVER pass content containing backticks (or `$(...)`, `$VAR`) to `python -c "..."` through bash. Three safe paths instead: (a) use the Write tool to land the payload as a file with zero bash interpretation surface, then a tiny Python call concatenates; (b) read the heredoc payload from a separate file with `python script.py --in payload.txt`; (c) when the file already exists, use the Edit tool (no bash, no quoting layer). The corrupted-write recovery cost on this single addendum was three full turns — much more expensive than the prevention.

**Failure mode this prevents.** Double-applied cherry-pick (empty-commit error or half-applied index), false-positive "the tree is corrupted, let me try harder" loops, and silently-corrupted lesson captures where the very document meant to prevent the next incident contains the failure mode it was describing.

**Recurrence — same-cycle 3rd hit (deep-tree `find`).** Immediately after the cherry-pick recovery, `find ~/.claude -name 'scaffold-auditor*'` against the global Power-Pack tree returned the same opaque internal-error. Read-only op, no mutation risk, but stacks with Addendum 14 (`ls | xargs basename`) and the two triggers above: **any Bash command that walks a deep global tree (`~/.claude`, the entire skills/agents/commands/vault constellation) risks an oversized result-frame and an opaque drop.** Use Glob (purpose-built, no shell, no streaming) for file lookup and targeted Grep for content. Bash is the wrong tool for wide tree enumeration — the cost is consistent in this codebase, not occasional.

**Meta — the entire isolation cycle was a no-op.** After all the worktree-add, cherry-pick, rebase, and verification work above, `git rev-list --count feat/rtk-next-clean ^origin/feat/rtk-compressor-fusion` returned **0** — the cherry-picked commit (`bae367b`) was ALREADY on origin. The parallel stream had pushed a 60-commit bundle that included my RTK commit along with its own onboarding work; the cycle-3 "B verdict, push halted" was honest about the WORKING-TREE contamination but the COMMIT-LEVEL push had effectively been completed by the sibling pane minutes later. The single-line preflight that would have ended the cycle in 5 seconds: `git fetch origin <branch> && git log --oneline origin/<branch> | grep <my-commit-sha>`. If the commit is already on origin, there is nothing to unblock — the worktree contamination is a sibling-pane problem, not a push problem. **Rule (Cherry-Pick Preflight):** before creating an isolation worktree for the purpose of pushing a single clean commit, fetch latest origin and check whether that commit is already on the target remote branch. The Power Pack is a multi-pane codebase where peer panes routinely push your in-flight commits as part of their own bundle; assuming "my commit is local until I push it" is wrong roughly 1 cycle in 5.


## Addendum 17 — Apex Onboarding Standard sealed (2026-05-19)

The Power Pack globalization arc (Phases A-E) shipped as 5 sibling commits between 2026-05-19 14:00 and 17:40: drift_report (A0), learning-sentinel sync (A1), runtime-hooks canonicalize (A2), agents/commands inventory (A3), global installer (B), verify_spp umbrella (C), 5-minute onboarding INSTALL.md (D), clean-machine E2E via Path-A HOME-redirect (E). Phase F formalizes the new doctrine: **no feature is DONE until a new-user `git clone` reaches S++ via `verify_full_install.py` (or the broader `verify_spp.py` umbrella) in under 5 minutes.**

**Why now.** Before this arc, the Power Pack was an Owner-onboarded artifact — every host required hand-tuned settings.json edits, manual hook copying, and undocumented sequencing. The 2026-06-15 Anthropic programmatic-budget cutover forced the question: can a new operator pick this up cold and get the same RTK/JIT savings as the reference host? The answer required a real installer + umbrella verifier + 5-minute doc + clean-machine proof. All four exist now.

**Operational lesson — multi-pane coordination.** This session burned three Owner-correction cycles inside a 30-minute window because the executing pane (this one) did not survey sibling commits before acting on a stale Owner answer. By the time the Owner answered "rename + new umbrella" for `verify_full_install.py`, the sibling pane had already shipped `verify_spp.py` as a separate umbrella that preserves `verify_full_install.py` as the programmatic-budget audit. Both designs were valid; sibling's landed first; the duplicate work was caught and reverted, not committed. The vaccine: `git log --oneline -10` before every Owner-answered execution is the cheapest collision detector. Saved as `feedback_check_sibling_commits_before_refactor.md`.

**Cross-cycle hang vaccine.** Same session also surfaced three "perceived hang" failures: a classifier deny on a PP→loose hook copy (the A1/A2 law is loose→PP, not the reverse), and two consecutive `[Tool result missing due to internal error]` on the same compound Bash chain. The unified rule: the turn never ends ambiguously on an anomalous tool result. Probe-or-surface in the same emission, and switch tool surface (atomic Read+Write+rm instead of compound `git mv && ...`) when an internal-error repeats on the same shape. Saved as `feedback_mirror_sync_direction_and_hooks_dir_deny.md` with the three sub-laws.

**Onboarding gate.** The Apex Onboarding Standard adds one absolute clause to the existing S+ Criteria: a feature shipping new components into the Power Pack must include the umbrella row + the install-global step + the INSTALL.md section that exercises it from a fresh clone. The DONE criterion is `verify_full_install.py` exit 0 (or `verify_spp.py` exit 0 for the broader S++ umbrella) after the documented 5-step new-user flow completes — measured wall-clock, not aspirational. Pre-2026-06-15 features are not retroactively rejected, but post-cutover work that omits the onboarding gate IS rejected at PR review.

## Addendum 18 — 2026-05-20: User-facing-silence anti-pattern after a tool internal-error

- **Scope.** Complementary to Addendum 16's *technical* Bash-trap rule and to the Addendum 17 cross-cycle-hang vaccine ("the turn never ends ambiguously on an anomalous tool result"). This one is narrower: the *interface* discipline when the previous tool call returned `[Tool result missing due to internal error]` and there is no queued next step.
...
## 2026-05-21 - /restart Path 2 + cmd.exe pane (2 bugs in one trace)

User-reported: `/restart` in a cmd.exe pane produced
`^[[200~claude --resume "..."^[[201~` followed by
`"claude" no se reconoce como un comando interno o externo`. Two bugs
combined into one visible failure:

1. **Bracketed-paste leak on cmd.exe.** `Set-Clipboard + SendKeys('^v')`
   triggers the terminal's paste pipeline. claude.exe enables bracketed-paste
   mode (`ESC[?2004h`) via VT100 while running; after claude.exe dies, cmd.exe
   inherits the terminal state but does NOT consume the `ESC[200~` / `ESC[201~`
   markers. The paste arrives as literal escape sequences wrapping the real
   command. **Vaccine:** type the command with `SendKeys.SendWait($cmd)`
   character-by-character. Typing is a key-stream, never engages bracketed-paste.
   Escape SendKeys metacharacters `+ ^ % ~ ( ) { } [ ]` with
   `-replace '([\+\^\%\~\(\)\{\}\[\]])', '{$1}'` for paths like
   `C:\Program Files (x86)\`.

2. **`claude` shim not on cmd.exe PATH.** Git Bash has a `claude` shim;
   cmd.exe and PowerShell often do NOT (USERPROFILE\.local\bin not in %PATH%).
   Pasting bare `claude --resume ...` works in Bash, fails in cmd. **Vaccine:**
   resolve the absolute exe path from the live parent process via
   `(Get-Process -Id $claudePid).Path` BEFORE Stop-Process kills it.
   Belt+suspenders: also re-export `CLAUDE_CODE_EXECPATH` so the detached
   SendKeys helper sees it regardless of env-inheritance chain (direct
   powershell spawn from claude.exe does NOT inherit CLAUDE_CODE_EXECPATH;
   the slash-command -> bash -> powershell chain DOES).

3. **General rule for terminal-pane-driven dispatch on Windows.** Never assume
