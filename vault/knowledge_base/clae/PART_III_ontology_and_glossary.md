---
title: "CLAE Part III — Ontology and Glossary"
family: clae
part: III
depends_on: [I, II]
feeds: [IV, V, VI, VII, IX, X, XIII, XVI, XVIII, XXII, XXIII]
status: SEALED
date: 2026-07-26
---

# Part III — Ontology and Glossary

## 1. Purpose

Parts I and II established the defect and its replacement primitive. Both leaned on words —
reference, delta, distance, residual, floor, bar, oracle, probe — that were used precisely
but never defined. This Part defines them, and it defines them as an *ontology* rather than
a word list: each term is fixed by its boundary against the term it is most often confused
with, because that boundary is where the institutional damage happens.

Every subsequent Part in this family is written in this vocabulary. Every rule, trap and eval
seeded downstream cites these terms. A reader who disagrees with a definition here should
raise it against this Part rather than redefining the term locally, because a term that means
two things in one family is worse than a term that means the wrong thing consistently.

## 2. Why an ontology is load-bearing rather than decorative

A vocabulary is not a convenience layer over the concepts. It is the boundary of what the
system can express, and therefore the boundary of what it can detect.

Part II established that a residual of zero is meaningless without the observing instrument's
coverage, because an instrument reports nothing remaining both when nothing remains and when
it has no vocabulary for what remains. That principle applies to institutional vocabulary
exactly as it applies to instrument vocabulary. **If this family lacks a word for a
distinction, that distinction becomes unreportable, and unreportable is operationally
identical to absent.**

The sharpest instance is the pair *undefined* and *unmeasured by choice*. Without two words
these are one state, and a decision with an owner and a date becomes indistinguishable from a
fact about the world. No amount of care by an individual author recovers a distinction the
shared vocabulary cannot carry. The words are the mechanism.

The second reason is adversarial rather than expressive. A closed loop reasserts itself
through vocabulary drift: the words of distance discipline are retained while their referents
are quietly replaced by internal objects. The reports still read correctly. Fixed definitions
with named boundaries are what make that drift detectable, and §9 lists the specific
...
- **Is** — the definition, stated positively.
- **Is not** — the nearest thing it is regularly mistaken for.
- **Boundary** — the test that separates it from that neighbour.
- **Confusion failure** — what breaks institutionally when the boundary is crossed.
- **Owner** — the Part or module that governs it in depth.
...
- **T-CLAE-VOCABULARY-DRIFT** — the terms of distance discipline are retained while their
  referents are replaced by internal objects; reports read correctly and measure nothing
  external.
- **T-CLAE-LOCAL-REDEFINITION** — a Part or module redefines a term this ontology owns,
  producing two meanings in one family and silently invalidating cross-Part citations.
- **T-CLAE-ORACLE-EXHAUSTION** — mechanical questions routed to an oracle until answers stop
  being considered, degrading the one channel that cannot be replaced.

## 13. Rule seeds — for Part XXIII

- **PR-CLAE-ONE-MEANING** — a term defined in this Part carries that meaning throughout the
  family. A Part needing a different concept names a new term and adds it here rather than
  narrowing an existing one locally.
- **PR-CLAE-PROVENANCE-BEFORE-REFERENCE** — the word *reference* may be used only where
  provenance and version are recorded. Absent either, the object is a criterion and is named
  as one.
- **PR-CLAE-ORACLE-TRIAGE** — before routing a question to an oracle, establish that no
  procedure could answer it in principle. Mechanical questions are answered by instruments.

## 14. Eval seeds — for Part XXIV

- **Drift probe.** Sample the family's own artifacts for each banned usage in §9. The family
  violating its own ontology is the earliest available signal of vocabulary drift.
- **Provenance probe.** For every use of *reference*, verify a recorded provenance and
  version. Uses without either are reclassified as criteria and the surrounding claims
  re-read.
- **Boundary probe.** For each pair in §8, construct one case sitting on the boundary and
  confirm the stated test resolves it. A test that cannot resolve its own boundary case is
  not a usable test.

## 15. Pseudoflow — auditing a document against this ontology

Read the document once for its claims. For each occurrence of a Tier-one or Tier-three term,
ask which entry in §4 or §6 the author meant, using the boundary test rather than the
surrounding prose.

Where the term is *reference*, look for a named external instance with provenance and a
version. Absent either, mark the occurrence as a criterion and re-read every conclusion that
depended on it, because those conclusions describe conformance rather than distance.

Where the term is *distance*, apply the saturation test. Where the value is anchored to an
internally-authored maximum, relabel it as a graded score and re-read the conclusions that
treated it as evidence about the external world.

Where a residual is reported as zero, look for the coverage declaration. Absent one,
reclassify the entry as undefined rather than treating it as evidence of health.

Where a term from §10 appears, confirm the document uses the canonical owner's meaning and
has not silently narrowed it.

Record each reclassification as a finding against the document, not as an edit to this
ontology. The ontology changes only through the addition of a new term, never through the
quiet widening of an existing one to accommodate a usage that was already published.

## 16. Integration

This ontology is the vocabulary the remaining Parts are written in, and the registries in
Parts XXII through XXV inherit its naming conventions from §11. Where a downstream Part needs
a concept this vocabulary lacks, the correct move is to add an entry here and cite it, which
keeps the family's expressible distinctions in one auditable place.

Outside the family, the terms in §10 are consumed unchanged from their canonical owners. The
integration map of Part XXVI resolves each of those relationships explicitly and is the place
any disagreement about ownership is settled.

## 17. Open questions

1. Is the instrument-versus-oracle boundary decidable in advance, or only in retrospect? The
   test in §7 asks whether a procedure could answer the question in principle, and "in
   principle" is doing considerable work. Part XVI must either sharpen it or accept that the
   boundary is negotiated per question class. — HYPOTHESIS.
2. Does the reference-versus-criterion distinction survive in domains with no external
   instances at all? If a domain has no exemplars, the vocabulary permits only *undefined*,
   which is honest but may make the family inapplicable exactly where quality matters most.
   Part IV must bound this. — UNKNOWN.
3. Should *deviation* and *disposition* share a lifecycle? Both are records of a decision with
   an owner, and maintaining two lifecycles may produce drift between them. — UNKNOWN.

## 18. Institutional writeback

This Part contributes three trap seeds, three process-rule seeds and three eval seeds, plus
the confusion matrix of §8, which is the portable artifact: any stack adopting distance
discipline will collapse those same ten pairs, and the detection column is usable
immediately, before any of this family's machinery exists.

The banned-usage list of §9 is written to be applied to this family first. A vocabulary
discipline exempting its own authors is the trap of Part I reproduced at the level of
language, and §14's drift probe exists to make that self-application observable rather than
merely intended.
