---
title: "CLAE — Evidence Index"
family: clae
type: registry
kind: extract
sources: [all 26 Parts, `Evidence` sections and epistemic labels]
derivation: mechanical extraction from the sealed Parts; no entry carries information absent from its source
status: POPULATED
date: 2026-08-10
---

# CLAE — Evidence Index

> **What this file is.** Part XXVI §5 defines the twelve companion artifacts as *"extracts for retrieval convenience"* — the schemas, consolidations and measured counts live in the Parts. This file locates entries; it does not restate, resolve or extend them.
> **Reading rule.** Every row cites the Part that seeded it. Where a row's source is ambiguous, the ambiguity is transcribed rather than resolved.

The charter requires an *evidence index complete* for completion. This file records, per Part, the evidence section it carries and the distribution of epistemic labels in its text.

## 1. Evidence sections

| Part | Evidence section | Subject |
|---|---|---|
| I | §5 | the PP instance |
| II | §10 | this stack's instruments re-read as retention decisions |
| III | — | **no evidence section** |
| IV | §10 | this stack's existing reference relationships |
| V | §11 | a compliant acquisition, from this compendium's own construction |
| VI | §10 | extraction levels across this stack |
| VII | §13 | ranking surfaces in this stack |
| VIII | §11 | correction loops in this stack |
| IX | §11 | ledger-shaped surfaces in this stack |
| X | §9 | this stack's existing floors |
| XI | §11 | derived and imposed floors in this stack |
| XII | §10 | Phase Zero in this stack |
| XIII | §11 | this stack's instruments by kind |
| XIV | §11 | instruments built in response to blockers in this stack |
| XV | §11 | incident conversion in this stack |
| XVI | §10 | oracle surfaces in this stack |
| XVII | §6 | — |
| XVIII | §10 | deviation surfaces in this stack |
| XIX | §4 | — |
| XX | §11 | closure surfaces in this stack |
| XXI | — | **no evidence section** |
| XXII | §9 | the registry pattern in this stack |
| XXIII | §8 | the rule archive in this stack |
| XXIV | §9 | a negative control, run |
| XXV | §9 | gate surfaces in this stack |
| XXVI | — | **no evidence section** |

**23 of 26 Parts carry an evidence section.** The 3 without one are III, XXI, XXVI. Part III is the ontology and Part XXVI is the integration map, so neither makes a first-order empirical claim about the stack. Part XXI reasons over the evidence the prior twenty Parts established rather than adding its own — which is consistent with its `depends_on: all prior`, and is the one absence a reader should check rather than assume.

## 2. Epistemic label distribution

| Part | VERIFIED | OBSERVED | INFERRED | HYPOTHESIS | REJECTED | UNKNOWN |
|--- |
|--- |
|--- |
|--- |
|--- |
|--- |
|---|
| I | 1 | 9 | 0 | 2 | 0 | 0 |
| II | 2 | 1 | 2 | 1 | 0 | 3 |
| III | 0 | 0 | 0 | 1 | 0 | 2 |
| IV | 0 | 2 | 3 | 1 | 0 | 2 |
| V | 0 | 1 | 1 | 1 | 0 | 2 |
| VI | 0 | 1 | 1 | 2 | 0 | 1 |
| VII | 0 | 1 | 1 | 1 | 0 | 2 |
| VIII | 0 | 1 | 1 | 1 | 0 | 2 |
| IX | 0 | 1 | 1 | 1 | 0 | 2 |
| X | 0 | 1 | 1 | 1 | 0 | 2 |
| XI | 0 | 1 | 1 | 2 | 0 | 2 |
| XII | 0 | 2 | 1 | 1 | 0 | 2 |
| XIII | 0 | 1 | 1 | 1 | 0 | 2 |
| XIV | 0 | 1 | 1 | 1 | 0 | 2 |
| XV | 0 | 1 | 1 | 1 | 0 | 2 |
| XVI | 0 | 1 | 1 | 1 | 0 | 2 |
| XVII | 0 | 2 | 1 | 1 | 0 | 2 |
| XVIII | 0 | 1 | 1 | 1 | 0 | 2 |
| XIX | 0 | 1 | 1 | 2 | 0 | 1 |
| XX | 0 | 1 | 1 | 1 | 0 | 2 |
| XXI | 0 | 1 | 1 | 1 | 0 | 2 |
| XXII | 0 | 1 | 2 | 1 | 0 | 2 |
| XXIII | 0 | 1 | 1 | 1 | 0 | 2 |
| XXIV | 0 | 2 | 1 | 2 | 0 | 1 |
| XXV | 0 | 1 | 1 | 1 | 0 | 2 |
| XXVI | 0 | 0 | 0 | 0 | 0 | 0 |
| **total** | **3** | **36** | **27** | **30** | **0** | **46** |

Counts are label *occurrences* in the Part text, not distinct claims: a Part discussing what INFERRED means increments the same counter as one labelling a claim. The column that matters for the charter is the ratio, and Part XXIII §6 states it in prose for the rule set — *"largely deductive"* — which this table supports and does not quantify per claim.

> **Measurement debt.** A complete evidence index would list every labelled claim and its supporting observation. That is a per-claim extraction the seeds do not support, and it is recorded as open rather than approximated.
