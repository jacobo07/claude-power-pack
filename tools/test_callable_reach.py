...
def gate_real_repo_positive_control() -> None:
    """The sweep must find a plausible population and BOTH poles in the real repo.

    Without this, a resolver that had stopped resolving would report an empty or
    all-one-state result, and every synthetic gate above would still pass.
    """
    rows = cr.scan()
    states = {r["state"] for r in rows}
    called = sum(1 for r in rows if r["state"] == cr.CALLED)
    unreached = sum(1 for r in rows if r["state"] in cr.UNREACHED)
    if len(rows) >= 500 and called >= 50 and unreached >= 50 \
            and {cr.CALLED, cr.NEVER} <= states:
        ...
        _fail("V-CALLABLE-REAL-REPO-CONTROL",
              f"implausible sweep: {len(rows)} symbols, {called} called, "
              f"{unreached} unreached, states={sorted(states)}")
