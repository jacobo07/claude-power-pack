...
def _resolve_pp_source(repo: Path, kind: str, name: str) -> Path:
    ...
    the repo, assuming the domain's own directory.

    Kept for callers that already know the root. It is NOT the installer's
    population source: `agents` is fed by three repo roots and this answers
    for one, so it reports MISSING-SOURCE for 7 of the 8 agents the
    inventory names. Use `_repo_population`.
    """
    return repo / kind / name


def _repo_population(repo: Path, kind: str) -> dict[str, Path] | None:
    """Every shippable <kind> file the repo holds, across ALL its roots.

    Enumerated by `modules.mirror_discovery`, which is the estate's owner
    of "which repo directories feed this live domain" and already knows
    that `agents` is fed by agents/, vault/agents/ and vendor/rtk/agents/.
    Deriving the population here instead of reading a hand-written list is
    the same repair PR-COVERAGE-BY-CONSTRUCTION-001 forced at the file
    level and then at the domain-root level: a set someone typed cannot
    grow when the estate does.

    Returns rel-name -> source path, precedence-first where several roots
    claim one name. Returns None -- never an empty dict -- when discovery
    cannot be reached, because "no agents exist" and "I could not ask" are
    different answers and only one of them is safe to install under.
    """
    if str(repo) not in sys.path:
        sys.path.insert(0, str(repo))
    try:
        from modules.mirror_discovery import discovery as _md
    except ImportError as e:
        print(f"install-global: cannot reach mirror_discovery ({e}); "
              f"refusing to guess the population", file=sys.stderr)
        return None
    pattern = dict(_md.DOMAINS).get(kind)
    if not pattern:
        return {}
    sources = _md.repo_sources(repo, kind, pattern)
    # Keyed by the LIVE name, because that is what the install writes and
    # what the inventory declares. Some pairs differ by name and only
    # discovery's ALIASES records that they are one document: keying by the
    # repo name would install a SECOND file under the repo's spelling and
    # leave the live one it was meant to update untouched.
    to_live = {v.split("/", 1)[-1]: k.split("/", 1)[-1]
               for k, v in _md.ALIASES.items()
               if k.startswith(f"{kind}/") and v.startswith(f"{kind}/")}
    return {to_live.get(rel, rel): paths[0]
            for rel, paths in sources.items() if paths}


def _sovereignty_exclusions(inv: dict, kind: str) -> dict[str, str]:
    """Names this repo must never install, with the reason, from the
    inventory's own declarations.

    The manifest keeps this job after losing the population job: it is the
    only artifact that records WHO OWNS a file, and ownership cannot be
    discovered by scanning a directory. Widening discovery without this
    would absorb a foreign estate into PP's install.
    """
    out: dict[str, str] = {}
    block = inv.get(kind, {}) or {}
    for entry in block.get("user_personal", []) or []:
        name = entry.get("name") if isinstance(entry, dict) else entry
        if name:
            out[name] = "user_personal: " + (
                (entry.get("redact_reason") or "operator-owned")
                if isinstance(entry, dict) else "operator-owned")
    for plugin, names in (block.get("plugins", {}) or {}).items():
        for name in names or []:
            out[name] = f"plugin-owned ({plugin})"
    return out
...
def main() -> int:
    ...
    # The population is DISCOVERED; the inventory declares only sovereignty.
    #
    # Until 2026-09-14 this read `inv[kind]["items"]`. No inventory file on
    # disk has ever had an `items` key -- they carry pp_original / plugins /
    # user_personal -- so the loop body never executed once. Every counter
    # therefore read zero, including the skipped-not-pp branch that would
    # have made it visible, and the install returned 0 with a clean report
    # while shipping nothing. Reading the schema the files actually use
    # would have fixed the silence and kept the deeper defect: that list was
    # generated by hand on 2026-05-19, names 8 agents, and 7 of them do not
    # resolve. Discovery answers with what the repo HOLDS.
    shippable_items: list[tuple[str, str, Path]] = []   # kind, name, source
    rows_seen = 0
    for kind in SHIPPABLE_KINDS:
        population = _repo_population(repo, kind)
        if population is None:
            print(f"install-global: population for {kind} is unknown; "
                  f"refusing to install rather than install nothing",
                  file=sys.stderr)
            return 6
        excluded = _sovereignty_exclusions(inv, kind)
        rows_seen += len(population)
        for name, src in sorted(population.items()):
            if name in excluded:
                counters["skipped-not-pp"] += 1
                report.append({"kind": kind, "name": name,
                               "verdict": "skipped-not-pp",
                               "reason": excluded[name]})
                continue
            shippable_items.append((kind, name, src))
    ...
    counters["inventory-rows-seen"] = rows_seen

    for kind, name, src in shippable_items:
        dst = _resolve_loose_target(kind, name)
        verdict = _backup_then_overwrite(dst, src, args.dry_run, report)
        counters[verdict] = counters.get(verdict, 0) + 1
        if report and isinstance(report[-1], dict):
            # Provenance travels with the row. vendor/rtk/agents is
            # supply-chain and the inventory calls its two agents
            # pp_original; the install may ship them, but it must not
            # record them as this repo's own work.
            try:
                report[-1]["repo_root"] = src.relative_to(repo).parent.as_posix()
            except ValueError:
                report[-1]["repo_root"] = str(src.parent)

    # Rows the inventory CLAIMS as PP-original that discovery cannot find.
    # Not silently dropped: a name declared canonical with no source in any
    # root is the estate's own debt, and the install is where it surfaces.
    for kind in SHIPPABLE_KINDS:
        population = _repo_population(repo, kind) or {}
        for entry in (inv.get(kind, {}) or {}).get("pp_original", []) or []:
            name = entry.get("name") if isinstance(entry, dict) else entry
            if name and name not in population:
                counters["claimed-without-source"] = counters.get(
                    "claimed-without-source", 0) + 1
                report.append({"kind": kind, "name": name,
                               "verdict": "claimed-without-source",
                               "reason": "inventory lists it as pp_original; "
                                         "no repo root holds it"})
