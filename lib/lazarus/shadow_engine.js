 ...
 *
 * `restoreSelfToo` (default false): also un-shadow files of the form
 * <ownerSid>.jsonl.live-shadow-<anyOwner>. Stop hook passes true so
 * the exiting session always recovers its OWN transcript even if it
 * was shadowed by some other (now-dead) session that never ran its
 * own restore. Without this, an asymmetric crash leaves the survivor's
 * transcript permanently invisible until panic-restore runs.
 */
function restore({
  projectId,
  ownerSid,
  dryRun = false,
  restoreSelfToo = false,
} = {}) {
  if (!projectId || !ownerSid) {
  ...
  const selfPrefix = ownerSid + SHADOW_SUFFIX_PREFIX;

  let entries;
  try { entries = fs.readdirSync(projDir); } catch { return { restored, skipped }; }
  for (const f of entries) {
    let sid = null;
    if (f.endsWith(ownedSuffix)) {
      sid = f.slice(0, -ownedSuffix.length);
    } else if (restoreSelfToo && f.startsWith(selfPrefix)) {
      // <ownerSid>.jsonl.live-shadow-<otherOwner> — our own transcript
      // shadowed by someone else. Take it back.
      sid = ownerSid;
    } else {
      continue;
    }
