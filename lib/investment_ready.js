...
const ERROR_HANDLING_PATTERNS = {
  ...
  '.ts':  [/\btry\s*\{/, /\bcatch\s*\(/, /\bthrow\b/],
  '.tsx': [/\btry\s*\{/, /\bcatch\s*\(/, /\bthrow\b/],
  '.jsx': [/\btry\s*\{/, /\bcatch\s*\(/, /\bthrow\b/],
  '.go':  [/if\s+err\s*!=\s*nil/, /\brecover\s*\(/, /\bdefer\s+/],
  '.rs':  [/\?;/, /Result<[^>]*>/, /\bpanic!/, /match\s+.*\{/],
  '.java':[/\btry\s*\{/, /\bcatch\s*\(/, /\bthrows\s+/],
  '.kt':  [/\btry\s*\{/, /\bcatch\s*\(/, /\bthrow\b/],
  '.ex':  [/\brescue\b/, /\btry\s+do\b/, /\bcatch\b/],
  '.exs': [/\brescue\b/, /\btry\s+do\b/, /\bcatch\b/],
...
const LOG_PATTERNS = {
  ...
  '.ex':  [/Logger\.(info|warn|error|debug)/, /IO\.(puts|inspect)/],
  '.exs': [/Logger\.(info|warn|error|debug)/, /IO\.(puts|inspect)/],
