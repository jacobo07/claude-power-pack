...
/* Legacy exact-name list, retained for reference and for consumers that import it.
 * NOT used for lookup — see LICENSE_FILE_RX. */
const LICENSE_FILES = [
...
/*
 * Lookup is by case-insensitive PATTERN over the real directory listing, not by
 * probing a fixed list of exact names. Two real upstreams broke the exact-name
 * approach on a case-sensitive filesystem:
 *   - tailark/blocks ships LICENCE.md  (British spelling; plain LICENSE 404s)
 *   - nilbuild/driver.js ships `license` (lowercase, no extension)
 * On Windows fs.existsSync('LICENSE') matches `license` by accident, so the bug
 * is invisible locally and appears only in CI or on a contributor's Linux box —
 * as tier UNKNOWN on a repo that is plainly licensed. Enumerate, then match.
 */
const LICENSE_FILE_RX = /^(licen[cs]e|copying|copying\.lesser|unlicen[cs]e)(\.(md|txt|rst))?$/i;
...
function findLicenseFiles(dir) {
  ...
  return entries
    .filter(e => e.isFile() && LICENSE_FILE_RX.test(e.name))
    .map(e => e.name)
    // Sort for a deterministic fingerprint: the hash must not depend on the
...
function classify(targetDir) {
  ...
  for (const name of findLicenseFiles(targetDir)) {
...
module.exports = {
  ...
  detectFromPackageJson, detectFromPyproject, findLicenseFiles,
  OBLIGATIONS, REDISTRIBUTION_BY_TIER, RESTRICTION_CLAUSES,
  LICENSE_FILES, LICENSE_FILE_RX,
};
